...
function AppContent() {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'canvas': return <ProductCanvasView />;
      default: return <HomeView />;
    }
  };

  const isFullWidthView = currentView === 'diagrama' || currentView === 'canvas';
  ...
  return (
    <div className="min-h-screen flex text-ink bg-nude font-sans selection:bg-forest/20">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Header />
        <main className={`flex-1 flex flex-col w-full ${isFullWidthView ? 'p-0 max-w-none' : 'p-6 lg:p-8 max-w-[1400px] mx-auto'}`}>
          {renderView()}
        </main>
      </div>
      <JarvisChat />
